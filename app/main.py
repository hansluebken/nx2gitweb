...
from .ui import login, dashboard, servers, teams, sync, admin, profile, cronjobs, json_viewer, code_viewer, changes, chat
...
@ui.page('/chat')
def chat_page():
    """Ninox Chat - AI assistant for database questions"""
    token = app.storage.user.get(SESSION_TOKEN_KEY)
    if not token:
        ui.navigate.to('/login')
        return
